...
ccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      Type TiltedRingModel
        ...
        integer SD_Switch   !A switch to determine whether the code uses the surface density or log surface density
      end Type


      Type TiltedRingFittingOptions
        integer nRingsPerBeam
        ...
        real ParamRange(0:12)
        integer CyclicSwitch(0:12)
...
c      print*, "Setting up the logical flags for the"
c     &              ," tilted ring fitting options"

c       Initialize the counts to zero
      TRFO%nFittedConstantParams=0
      TRFO%nFittedRadialParams=0
c       Loop through all possible parameters
      do i=0,12
c           Check if the parameter is constant
        if(TRFO%ConstParams(i)) then
c           Check if the parameter is fixed
...
c       Now set the counts for the radial profiles
        else
            if(TRFO%FixedParams(i)) then
...
c       Calculate the total number of parameters to be fitted
