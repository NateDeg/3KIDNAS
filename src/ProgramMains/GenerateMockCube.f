...
c       Calculate the area of a pixel in arcsec^2
      pixelarea=ModelDC%DH%PixelSize(0)*ModelDC%DH%PixelSize(1)
c           First buld the model
      call BuildTiltedRingModel(ModelTiltedRing,idum) !/src/TiltedRingModelGeneration/TiltedRingModelGeneration.f
...
c       If being verbose output the velocity smoothed source cube
      if(OutputFileVolumeSwitch .eq. 2) then
...
c           This routine sets the units for the output cube header
      ...
      DC%DH%AxisType(0)='RA---TAN'
      DC%DH%AxisType(1)='DEC--TAN'
