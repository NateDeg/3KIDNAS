...
def MakeAllPlots(CatID,WallCat,StrDict,ParamDicts,ObsDicts):
    ...
    OutName=StrDict['BaseFileName']+"_"+NameSuffix+"_FitComparisons.png"
...
#fig.text(1.2,yTextLoc, LabelStr , ha='left',rotation=0,va='center',size=25,color='k')
    ...
    Key="VROT"
    KeywordPlot(fig,Key,placement,ParamDicts,StrDict,WallCat,Radius,CatID)
    ...
    placement=[left+(w+buf),base-1*(h+buf),w,h]
    Key="YCENTER"
    ax1=KeywordPlot(fig,Key,placement,ParamDicts,StrDict,WallCat,Radius,CatID)
...
def KeywordPlot(fig,Key,placement,ParamDicts,StrDict,WallCat,Radius,CatID):
    LW=1
    MW=10
    nFit=np.shape(ParamDicts)[0]
...
def CatLine(Key,WallCat,CatID):
    ...
    if Key == "INCLINATION":
        ...
        print("Inclination",WallCat.ell_min[CatID],WallCat.ell_maj[CatID],CatVal)
        print(np.log10(2.996857e9))
    elif Key == "POSITIONANGLE":
        drawCatVal='True'
